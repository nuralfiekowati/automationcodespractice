...
class RelevantSearchResultTest {

	WebElement element, element2;
	String productTitle = "";
	int j;

	public void relevantResult(WebDriver driver) {
		...
		if (productResultList != null && productResultList.size() > 0) {
			...
			for (int i = 0; i < productResultList.size(); i++) {
				...
				if (productTitle.contains("Dress") == true || productTitle.contains("dress") == true) {
					System.out.println("Relevant product " + j + ": " + productTitle);
				} else {
					System.out.println("Irrelevant product " + j + ": " + productTitle);
				}
